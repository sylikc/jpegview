 ...
 * 
 * CHANGES MADE:
 * - Replaced PIL-specific structs
 * - ImagingBcnDecode() writes to a 1D flattened array
 * - All uncompressed images are outputted as BGRA to accommodate JPEGView
 * - Removed edge case handling for lowest mipmap level support; we only show the first/biggest mipmap
 */
...
static void
bc1_color_load(bc1_color* dst, const uint8_t* src) {
    dst->c0 = LOAD16(src);
    dst->c1 = LOAD16(src + 2);
...
static void
decode_bc1_color(rgba* dst, const uint8_t* src, int separate_alpha) {
    bc1_color col;
    rgba p[4];
    int n, cw;
...
static void
decode_bc3_alpha(char* dst, const uint8_t* src, int stride, int o, int sign) {
    uint16_t a0, a1;
    uint8_t a[8];
    int n, lut1, lut2, aw;
    if (sign == 1) {
...
static void
decode_bc1_block(rgba* col, const uint8_t* src) {
    decode_bc1_color(col, src, 0);
}

static void
decode_bc2_block(rgba* col, const uint8_t* src) {
    int n, bitI, byI, av;
...
static void
decode_bc3_block(rgba* col, const uint8_t* src) {
    decode_bc1_color(col, src + 8, 1);
    decode_bc3_alpha((char*)col, src, sizeof(col[0]), 3, 0);
}

static void
decode_bc4_block(lum* col, const uint8_t* src) {
    decode_bc3_alpha((char*)col, src, sizeof(col[0]), 0, 0);
}

static void
decode_bc5_block(rgba* col, const uint8_t* src, int sign) {
...
static uint8_t
get_bits(const uint8_t* src, int bit, int count) {
    uint8_t v;
    int x;
    int by = bit >> 3;
    bit &= 7;
...
static uint8_t
expand_quantized(uint8_t v, int bits) {
    v = v << (8 - bits);
    return v | (v >> bits);
}
...
static void
decode_bc7_block(rgba* col, const uint8_t* src) {
    rgba endpoints[6];
...
static void
bc6_sign_extend(uint16_t* v, int prec) {
    int x = *v;
    if (x & (1 << (prec - 1))) {
        x |= -1 << prec;
    }
...
static int
bc6_unquantize(uint16_t v, int prec, int sign) {
    int s = 0;
    int x;
    if (!sign) {
        x = v;
...
static void
decode_bc6_block(rgba* col, const uint8_t* src, int sign) {
...
static void
put_block(uint8_t* dst, DecodeState *state, const char* col, int sz) {
    ...
    int chan = state->outNumChannels;

    for (int i = 0; i < blockSize; i++) {
        for (int j = 0; j < blockSize; j++) {
            // Single channel image edge case
            if (state->inNumChannels == 1) {
                int src_flat_idx = (i * blockSize * sz) + (j * sz);
                int dest_flat_idx = ((state->y + i) * width * chan) + ((state->x + j) * chan);
                
                dst[dest_flat_idx + 0] = col[src_flat_idx];
                dst[dest_flat_idx + 1] = col[src_flat_idx];
                dst[dest_flat_idx + 2] = col[src_flat_idx];
                dst[dest_flat_idx + 3] = 255;
                continue;
            }

            // Dual channel (BC5) edge case
            if (state->inNumChannels == 2) {
                int src_flat_idx_r = (i * blockSize * sz) + (j * sz) + 0;
                int src_flat_idx_g = (i * blockSize * sz) + (j * sz) + 1;
                
                // RGBA to BGRA
                int dest_flat_idx_r = ((state->y + i) * width * chan) + ((state->x + j) * chan) + 2;
                int dest_flat_idx_g = ((state->y + i) * width * chan) + ((state->x + j) * chan) + 1;
                int dest_flat_idx_b = ((state->y + i) * width * chan) + ((state->x + j) * chan) + 0;
                int dest_flat_idx_a = ((state->y + i) * width * chan) + ((state->x + j) * chan) + 3;
                
                dst[dest_flat_idx_r] = col[src_flat_idx_r];
                dst[dest_flat_idx_g] = col[src_flat_idx_g];
                dst[dest_flat_idx_b] = 0;
                dst[dest_flat_idx_a] = 255;
                continue;
            }

            int src_flat_idx_r = (i * blockSize * sz) + (j * sz) + 0;
            int src_flat_idx_g = (i * blockSize * sz) + (j * sz) + 1;
            int src_flat_idx_b = (i * blockSize * sz) + (j * sz) + 2;
            int src_flat_idx_a = (i * blockSize * sz) + (j * sz) + 3;
            ...
            dst[dest_flat_idx_r] = col[src_flat_idx_r];
            dst[dest_flat_idx_g] = col[src_flat_idx_g];
            dst[dest_flat_idx_b] = col[src_flat_idx_b];
            dst[dest_flat_idx_a] = col[src_flat_idx_a];
        }
    }
...
#define DECODE_LOOP(NN, SZ, TY, ...)                                    \
    case NN:                                                            \
        while (bytes >= SZ) {                                           \
            ...
            put_block(dst, state, (const char *)col, sizeof(col[0]));   \
            ptr += SZ;                                                  \
            bytes -= SZ;                                                \
            if (state->y >= ymax) {                                     \
                return 0;                                               \
        ...
        DECODE_LOOP(4, 8, lum);
    case 5: {
        int sign = strcmp(state->pixel_format, "BC5S") == 0 ? 1 : 0;
        while (bytes >= 16) {
            rgba col[16];
    ...
    case 6: {
        int sign = strcmp(state->pixel_format, "BC6HS") == 0 ? 1 : 0;
        while (bytes >= 16) {
            rgba col[16];
...
int
ImagingBcnDecode(const uint8_t* src, uint8_t* dst, DecodeState *state, uint32_t bytes) {
    return decode_bcn(src, dst, state, bytes);
}
